...
export default function StudentDashboard() {
  ...
  const handleFeedbackSuccess = () => {
    ...
    // Refresh subjects
    API.get("student-subjects/").then(r => setSubjects(r.data));
  };
  ...
  const submitted = subjects.filter((s) => s.feedback_submitted).length;
      ...
      <header className="sticky top-0 z-20 bg-surface-900/80 backdrop-blur-xl border-b border-surface-700/50">
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-surface-100 font-display">Student Dashboard</h1>
                ...
                {subjects.map((sub) => (
                  ...
                  >
                    <div className="min-w-0">
                      <h4 className="font-semibold text-surface-100">{sub.subject_name}</h4>
                      <p className="text-sm text-surface-400 mt-0.5">
                        <span className="text-surface-500">{sub.subject_code}</span> · {sub.teacher}
                      </p>
                    </div>
      ...
      <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="glass-card p-10 text-center max-w-sm animate-fade-in">
          ...
          <p className="text-surface-400 text-sm">
            Thank you for your feedback on {subject.subject_name}
          </p>
        </div>
      </div>
    );
