...
def get_student_subjects(request):
    ...
    Query: SubjectOffering.objects.filter(branch=student.student_profile.branch, semester=student.student_profile.semester)
    """
    ...
    # Use SessionOffering instead of base SubjectOffering for tracking
    session_offerings = SessionOffering.objects.filter(
        session=active_session,
        base_offering__branch=profile.branch,
        base_offering__semester=profile.semester
    ).select_related('base_offering__subject', 'base_offering__branch', 'base_offering__semester', 'teacher')

    from .models import SubmissionTracker
    data = []
    for so in session_offerings:
        # Correct way: Check SubmissionTracker using SessionOffering
        given = SubmissionTracker.objects.filter(
            student=request.user, 
            offering=so,
            session=active_session
        ).exists()
        
        teacher_name = (so.teacher.get_full_name() or so.teacher.username) if so.teacher else "Unassigned"

        data.append({
            "subject_id": so.base_offering.subject.id,
            "offering_id": so.id, # This is SessionOffering ID
            "subject_name": so.base_offering.subject.name,
            "subject_code": so.base_offering.subject.code,
            "teacher": teacher_name,
            "branch": so.base_offering.branch.name,
            "semester": so.base_offering.semester.number,
...
def _calculate_avg_rating(responses_queryset):
    """Calculate average rating across all RATING answers for a set of FeedbackResponses."""
...
def _calculate_category_averages(responses_queryset):
    """Calculate average rating per question category for a set of FeedbackResponses."""
    from .models import Answer
    cats = Answer.objects.filter(
        response_parent__in=responses_queryset,
        question__question_type='RATING'
    ).values('question__category').annotate(avg=Avg('rating'))
    result = {}
    for row in cats:
        cat = (row['question__category'] or 'GENERAL').lower()
        result[f'avg_{cat}'] = round(row['avg'] or 0, 2)
    return result
...
def teacher_dashboard(request):
    ...
    # Use SessionOffering (FeedbackResponse.offering points to SessionOffering)
    session_offerings = SessionOffering.objects.filter(teacher=request.user, is_active=True).select_related('base_offering__subject', 'base_offering__branch', 'base_offering__semester')
    view_mode = request.query_params.get('view', 'combined')
    data = []

    if view_mode == 'combined':
        subjects = Subject.objects.filter(offerings__session_offerings__in=session_offerings).distinct()
        for subject in subjects:
            feedbacks = FeedbackResponse.objects.filter(offering__in=session_offerings, offering__base_offering__subject=subject)
            avg_overall = _calculate_avg_rating(feedbacks)
            cat_avgs = _calculate_category_averages(feedbacks)
            data.append({
                "subject_id": subject.id,
                "subject_name": subject.name, "subject_code": subject.code,
                "feedback_count": feedbacks.count(), "performance": _get_performance_label(avg_overall if avg_overall > 0 else None),
                "sentiment_summary": _get_sentiment_summary(feedbacks),
                "avg_overall": avg_overall, **cat_avgs
            })
    else:
        for so in session_offerings:
            feedbacks = FeedbackResponse.objects.filter(offering=so)
            avg_overall = _calculate_avg_rating(feedbacks)
            cat_avgs = _calculate_category_averages(feedbacks)
            name_suffix = f" ({so.base_offering.branch.code} Sem {so.base_offering.semester.number})"
            data.append({
                "subject_id": so.id,
                "subject_name": so.base_offering.subject.name + name_suffix, "subject_code": so.base_offering.subject.code,
                "feedback_count": feedbacks.count(), "performance": _get_performance_label(avg_overall if avg_overall > 0 else None),
                "sentiment_summary": _get_sentiment_summary(feedbacks),
                "avg_overall": avg_overall, **cat_avgs
            })
...
def teacher_performance(request):
    if request.user.role != 'teacher': return Response({'error': 'Only teachers allowed'}, status=403)
    session_offerings = SessionOffering.objects.filter(teacher=request.user, is_active=True).select_related('base_offering__subject', 'base_offering__branch', 'base_offering__semester')
    all_feedback = FeedbackResponse.objects.filter(offering__in=session_offerings)
    
    overall_avg = _calculate_avg_rating(all_feedback)
    view_mode = request.query_params.get('view', 'combined')
    subject_performance = []
    ...
    if view_mode == 'combined':
        ...
        for subject in subjects:
            feedbacks = FeedbackResponse.objects.filter(offering__in=session_offerings, offering__base_offering__subject=subject)
            avg = _calculate_avg_rating(feedbacks)
            subject_performance.append({
                "subject_name": subject.name, "subject_code": subject.code,
                "avg_overall": avg, "feedback_count": feedbacks.count(),
                "performance": _get_performance_label(avg if avg > 0 else None),
            })
    else:
        for so in session_offerings:
            feedbacks = FeedbackResponse.objects.filter(offering=so)
...
def teacher_analytics(request):
    ...
    for so in session_offerings:
        ...
        avg = _calculate_avg_rating(feedbacks)
        data.append({
            "subject_name": f"{so.base_offering.subject.name} ({so.base_offering.branch.code} Sem {so.base_offering.semester.number})",
            "subject_code": so.base_offering.subject.code,
            "average_rating": avg if avg > 0 else None,
...
def teacher_performance_charts(request):
    ...
    if view_mode == 'combined':
        ...
        for subject in subjects:
            ...
            subject_values.append(avg)
    else:
        for so in session_offerings:
            feedbacks = FeedbackResponse.objects.filter(offering=so)
            avg = _calculate_avg_rating(feedbacks)
            name_suffix = f" {so.base_offering.branch.code} S{so.base_offering.semester.number}"
            subject_labels.append(so.base_offering.subject.name + name_suffix)
            subject_values.append(avg)

    cat_avgs = _calculate_category_averages(all_feedback)
    category_labels = ['Teaching', 'Punctuality', 'Clarity', 'Interaction', 'Behavior', 'General']
    category_values = [cat_avgs.get(f'avg_{k.lower()}', 0) for k in category_labels]
    # Remove categories with 0 values (no questions of that type)
    filtered = [(l, v) for l, v in zip(category_labels, category_values) if v > 0]
    if filtered:
        category_labels, category_values = zip(*filtered)
        category_labels, category_values = list(category_labels), list(category_values)

    # Rating distribution from Answer model
    all_answers = Answer.objects.filter(response_parent__in=all_feedback, question__question_type='RATING', rating__isnull=False)
    total = all_answers.count()
    excellent = all_answers.filter(rating__gte=4).count()
    good = all_answers.filter(rating__gte=3, rating__lt=4).count()
    average = all_answers.filter(rating__gte=2, rating__lt=3).count()
    poor = all_answers.filter(rating__lt=2).count()

    # Question-wise averages
    question_avgs = all_answers.values('question__text').annotate(avg=Avg('rating')).order_by('question__order')
    q_labels = [row['question__text'] for row in question_avgs]
    q_values = [round(row['avg'] or 0, 2) for row in question_avgs]

    # Performance trend (monthly) - Group in Python to avoid MySQL TZ issues
    try:
        from collections import defaultdict
        import datetime
        monthly_data = defaultdict(list)
        
        # Get all rating answers for these feedbacks
        ans_list = Answer.objects.filter(
            response_parent__in=all_feedback,
            question__question_type='RATING',
            rating__isnull=False
        ).values('rating', 'response_parent__submitted_at')
        
        for row in ans_list:
            sub_date = row['response_parent__submitted_at']
            if sub_date:
                month_key = sub_date.strftime('%Y-%m')
                monthly_data[month_key].append(row['rating'])
        
        sorted_months = sorted(monthly_data.keys())
        trend_labels = []
        trend_values = []
        for m_key in sorted_months[-6:]:
            dt = datetime.datetime.strptime(m_key, '%Y-%m')
            trend_labels.append(dt.strftime('%b %Y'))
            vals = monthly_data[m_key]
    ...
    return Response({
        ...
        'question_averages': {'labels': q_labels, 'values': q_values},
...
def feedback_window_manager(request):
    ...
    if request.method == 'GET':
        windows = FeedbackSession.objects.all().order_by('-start_date')
        serializer = None # (windows, many=True)
        return Response(serializer.data)

    elif request.method == 'POST':
        FeedbackSession.objects.all().update(is_active=False)
        serializer = None # (data=request.data)
...
def feedback_window_detail(request, pk):
    ...
    try:
        window = FeedbackSession.objects.get(pk=pk)
    except FeedbackSession.DoesNotExist:
        return Response({'error': 'FeedbackResponse window not found'}, status=404)

    if request.method == 'PUT':
        if request.data.get('is_active', False):
            FeedbackSession.objects.exclude(pk=pk).update(is_active=False)
        serializer = None # (window, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=400)
...
def current_feedback_window(request):
    try:
        window = FeedbackSession.objects.filter(is_active=True).first()
        if not window:
            return Response(
        ...
        if not (window.start_date <= now <= window.end_date):
            return Response(
                {'message': 'FeedbackResponse window is closed'},
                status=status.HTTP_400_BAD_REQUEST
            )
        serializer = None # (window)
        return Response(serializer.data)
    except Exception as e:
        return Response(
...
def hod_dashboard_overview(request):
    ...
    return Response({
        ...
        "session": {"name": active_session.name, "year": active_session.year} if active_session else None,
    })
...
def hod_teacher_detail(request, pk):
    ...
    # Use SessionOffering to find subjects for this teacher
    teacher_session_offerings = SessionOffering.objects.filter(teacher=teacher, is_active=True).select_related('base_offering__subject')
    subjects = Subject.objects.filter(offerings__session_offerings__in=teacher_session_offerings).distinct()
    subject_data = []

    all_feedback = FeedbackResponse.objects.filter(offering__in=teacher_session_offerings)
    overall_avg = _calculate_avg_rating(all_feedback)

    for subject in subjects:
        feedbacks = FeedbackResponse.objects.filter(offering__in=teacher_session_offerings, offering__base_offering__subject=subject)
        avg_overall = _calculate_avg_rating(feedbacks)
        cat_avgs = _calculate_category_averages(feedbacks)
        ...
        subject_data.append({
            ...
            **cat_avgs,
            'performance': _get_performance_label(avg_overall if avg_overall > 0 else None),
            'sentiment_summary': _get_sentiment_summary(feedbacks),
        })
...
def hod_send_report(request):
    ...
    teacher_sos = SessionOffering.objects.filter(teacher=teacher, is_active=True).select_related('base_offering__subject')
    subjects = Subject.objects.filter(offerings__session_offerings__in=teacher_sos).distinct()
    all_feedback = FeedbackResponse.objects.filter(offering__in=teacher_sos)
    overall_avg = _calculate_avg_rating(all_feedback)
    ...
    for subject in subjects:
        feedbacks = FeedbackResponse.objects.filter(offering__in=teacher_sos, offering__base_offering__subject=subject)
        avg = _calculate_avg_rating(feedbacks)
        sentiment = _get_sentiment_summary(feedbacks)
        performance = _get_performance_label(avg if avg > 0 else None)

        lines.append(f"\nSubject: {subject.name} ({subject.code})")
        lines.append(f"  Average Rating: {avg if avg > 0 else 'N/A'} / 5.0")
    ...
    lines.append(f"\n{'=' * 50}")
    lines.append(f"Overall Average: {overall_avg if overall_avg > 0 else 'N/A'} / 5.0")
    lines.append(f"Overall Performance: {_get_performance_label(overall_avg if overall_avg > 0 else None)}")
...
def hod_analytics(request):
    ...
    # Teacher ranking - use FeedbackResponse.teacher direct FK
    teachers = User.objects.filter(role='teacher', is_active=True)
    ranking = []
    for teacher in teachers:
        feedbacks = FeedbackResponse.objects.filter(teacher=teacher)
        avg = _calculate_avg_rating(feedbacks)
        if avg > 0:
            ranking.append({
                'id': teacher.id,
    ...
    # Subject performance - use SessionOffering path
    session_offerings = SessionOffering.objects.filter(is_active=True).select_related('base_offering__subject', 'teacher')
    subjects = Subject.objects.filter(offerings__session_offerings__in=session_offerings).distinct()
    subject_performance = []
    for subject in subjects:
        feedbacks = FeedbackResponse.objects.filter(offering__in=session_offerings, offering__base_offering__subject=subject)
        avg = _calculate_avg_rating(feedbacks)
        # Get teacher names from SessionOffering
        teacher_names_set = set()
        for so in session_offerings.filter(base_offering__subject=subject):
            if so.teacher:
                teacher_names_set.add(so.teacher.get_full_name() or so.teacher.username)
        teacher_names = ", ".join(teacher_names_set) if teacher_names_set else "Unassigned"
        subject_performance.append({
            'subject_name': subject.name,
            'subject_code': subject.code,
    ...
    # Rating distribution (1-5) from Answer model
    all_feedback = FeedbackResponse.objects.all()
    all_answers = Answer.objects.filter(response_parent__in=all_feedback, question__question_type='RATING', rating__isnull=False)
    rating_distribution = {}
    for i in range(1, 6):
        rating_distribution[str(i)] = all_answers.filter(rating=i).count()

    # Sentiment distribution
    sentiment_distribution = _get_sentiment_summary(all_feedback)

    # Department average
    dept_avg = _calculate_avg_rating(all_feedback)
...
def hod_statistics(request):
    ...
    # Use SessionOffering for subject-teacher mapping
    session_offerings = SessionOffering.objects.filter(is_active=True).select_related('base_offering__subject', 'teacher')
    subjects = Subject.objects.filter(offerings__session_offerings__in=session_offerings).distinct()
    stats = []
...
def hod_report(request):
    ...
    # Use SessionOffering for report data
    session_offerings = SessionOffering.objects.filter(is_active=True).select_related('base_offering__subject', 'base_offering__branch', 'base_offering__semester', 'teacher')
    report = []
    ...
    for so in session_offerings:
        ...
        report.append({
            "subject": f"{so.base_offering.subject.name} ({so.base_offering.branch.code} Sem {so.base_offering.semester.number})",
            "subject_code": so.base_offering.subject.code,
            "teacher": teacher_name,
...
def hod_export_report_pdf(request):
    ...
    p.drawCentredString(width / 2, height - 50, "Student FeedbackResponse Report")
    ...
    for so in session_offerings:
        ...
        sentiment = _get_sentiment_summary(feedbacks)
        suggestion = get_improvement(avg_overall if avg_overall > 0 else None)
        ...
        suffix = f"({so.base_offering.branch.code} Sem {so.base_offering.semester.number})"
        p.drawString(60, y, f"Subject: {so.base_offering.subject.name} {suffix}")
        y -= 18
        ...
        y -= 16
        p.drawString(80, y, f"Overall Avg: {avg_overall}")
        y -= 16
        # Dynamic category display
        cat_display = "  |  ".join(f"{k.replace('avg_', '').title()}: {v}" for k, v in cat_avgs.items())
        if cat_display:
            p.drawString(80, y, cat_display)
            y -= 16
...
def hod_teacher_report(request, pk):
    ...
    all_feedback = FeedbackResponse.objects.filter(offering__assignment__teacher=teacher, offering__assignment__is_active=True)
    
    overall_avg = all_feedback.aggregate(avg=Avg('sentiment_score'))['avg']
    total_feedback = all_feedback.count()
    ...
    for i in range(1, 6):
        rating_dist[str(i)] = all_feedback.filter(
            sentiment_score__gte=i - 0.5, sentiment_score__lt=i + 0.5
        ).count()
    ...
    cat_agg = all_feedback.aggregate(
        punctuality=Avg('sentiment_score'),
        teaching=Avg('sentiment_score'),
        clarity=Avg('sentiment_score'),
        interaction=Avg('sentiment_score'),
        behavior=Avg('sentiment_score'),
    )
...
def hod_department_report(request):
    ...
    teachers_list = []
    for teacher in teachers:
        fbs = FeedbackResponse.objects.filter(offering__assignment__teacher=teacher, offering__assignment__is_active=True)
        avg = fbs.aggregate(avg=Avg('sentiment_score'))['avg']
    ...
    dept_avg = all_feedback.aggregate(avg=Avg('sentiment_score'))['avg']
    total_feedback = all_feedback.count()
    ...
    subject_perf = []
    for subject in subjects:
        fbs = FeedbackResponse.objects.filter(offering__subject=subject)
        avg = fbs.aggregate(avg=Avg('sentiment_score'))['avg']
        subject_perf.append({
            "name": subject.name,
            "code": subject.code,
    ...
    recent_avg = recent_feedbacks.aggregate(avg=Avg('sentiment_score'))['avg'] or 0
    older_avg = older_feedbacks.aggregate(avg=Avg('sentiment_score'))['avg'] or 0
...
def list_enrollments(request):
    """List all enrollments (HOD/Admin only) based on Branch/Semester enrollment."""
    if request.user.role not in ('hod', 'admin'):
        return Response({'error': 'Only HOD/Admin can view enrollments'}, status=403)

    from .models import FeedbackSession, StudentSemester, SessionOffering, Subject
    from django.utils import timezone
    
    # Get active session
    current_session = FeedbackSession.objects.filter(is_active=True).first()
    ...
    offerings_qs = SessionOffering.objects.filter(
        session=current_session, 
        is_active=True
    ).select_related('base_offering__subject', 'base_offering__branch', 'base_offering__semester')

    if subject_id:
        offerings_qs = offerings_qs.filter(base_offering__subject_id=subject_id)

    # Group subjects by (branch_id, semester_id) for efficient mapping
    subjects_by_group = {}
    for off in offerings_qs:
        key = (off.base_offering.branch_id, off.base_offering.semester_id)
        if key not in subjects_by_group:
            subjects_by_group[key] = []
        subjects_by_group[key].append(off.base_offering.subject)

    # Get all students enrolled in this session
    student_semesters = StudentSemester.objects.filter(
        session=current_session,
        is_active=True
    ).select_related('student', 'branch', 'semester')

    data = []
    for ss in student_semesters:
        key = (ss.branch_id, ss.semester_id)
        subjects = subjects_by_group.get(key, [])
        
        for subject in subjects:
            data.append({
                'id': f"{subject.id}-{ss.student.id}",
                'student': ss.student.id,
                'subject': subject.id,
                'student_name': ss.student.get_full_name() or ss.student.username,
                'student_enrollment_no': ss.student.enrollment_no,
                'subject_name': subject.name,
                'subject_code': subject.code,
                'created_at': now,
            })
...
def enrollment_form_data(request):
    ...
    # Pre-fetch their active semesters for the current session (or any active session if none provided)
    from .models import StudentSemester
    if session_id:
        semesters = StudentSemester.objects.filter(is_active=True, session_id=session_id).select_related('branch', 'semester')
    else:
        semesters = StudentSemester.objects.filter(is_active=True).select_related('branch', 'semester')
        
    sem_dict = {s.student_id: s for s in semesters}
    ...
    for s in students:
        s_data = {
            ...
            'full_name': s.get_full_name().strip() or s.username,
            'enrollment_no': s.enrollment_no or s.username,
            'is_first_login': s.is_first_login,
            'student_profile': None
        }
        
        sem = sem_dict.get(s.id)
        if sem:
            s_data['student_profile'] = {
                'branch_code': sem.branch.code,
                'semester_number': sem.semester.number
            }
            
        student_data.append(s_data)

    offerings = SubjectOffering.objects.select_related('subject', 'branch', 'semester', 'assignment__teacher').all()
    offering_data = []
    for o in offerings:
        teacher_name = "-"
    ...
    return Response({
        ...
        'subjects': offering_data, # Named 'subjects' for frontend compatibility
    })
...
class DepartmentViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """Only HOD/Admin can modify departments"""
        if self.action in ['create', 'update', 'partial_update', 'destroy']:
            if self.request.user.role not in ['hod', 'admin']:
                raise PermissionDenied("Only HOD/Admin can manage departments")
...
def student_dashboard(request):
    ...
    from .models import SessionOffering, SubmissionTracker, FeedbackSession
    active_session = FeedbackSession.objects.filter(is_active=True).first()
    if not active_session:
        return Response({'error': 'No active feedback session found'}, status=404)
    
    # Get session-specific offerings
    offerings = SessionOffering.objects.filter(
        session=active_session,
        base_offering__branch=profile.branch,
        base_offering__semester=profile.semester
    ).select_related(
        'base_offering__subject', 'base_offering__branch', 'base_offering__semester', 'teacher'
    )
    ...
    for so in offerings:
        ...
        if so.teacher:
            ...
            teacher_info = {
                'id': so.teacher.id,
                'name': so.teacher.get_full_name() or so.teacher.username,
                'username': so.teacher.username
            }
        
        # Check if feedback already given
        feedback_given = SubmissionTracker.objects.filter(
            student=request.user,
            offering=so,
            session=active_session
        ).exists()
...
def teacher_assignments(request):
    ...
    for assignment in assignments:
        ...
        feedback_count = FeedbackResponse.objects.filter(offering=offering).count()
...
def get_offering_details(request, pk):
    ...
    enrolled_students = User.objects.filter(
        role='student',
        student_semesters__branch=offering.branch,
        student_semesters__semester=offering.semester,
        student_semesters__is_active=True
    ).distinct().count()
...
def manage_teachers(request):
    """
    HOD/Admin: List and manage teachers
    """
    if request.user.role not in ['hod', 'admin']:
        return Response({'error': 'Only HOD/Admin can access teacher management'}, status=403)
    ...
    if request.method == 'GET':
        ...
        if user.role == 'hod' and getattr(user, 'department', None):
            teachers = teachers.filter(department=user.department)
            
        data = []
        for teacher in teachers:
            subject_count = SessionOffering.objects.filter(teacher=teacher, is_active=True).count()
            data.append({
                'id': teacher.id,
                ...
                'username': teacher.username,
                'email': teacher.email,
                'department': teacher.department.id if teacher.department else None,
        ...
        # Reuse TeacherCreateSerializer for creating new teachers
...
def teacher_detail(request, pk):
    """
    HOD/Admin: View, update or delete a teacher
    """
    if request.user.role not in ['hod', 'admin']:
        return Response({'error': 'Only HOD/Admin allowed'}, status=403)
    ...
    if request.method == 'GET':
        ...
        if dept_id:
            try:
                teacher.department = Department.objects.get(pk=dept_id)
            except Department.DoesNotExist:
                return Response({'error': 'Department not found'}, status=400)
        teacher.designation = request.data.get('designation', teacher.designation)
        if 'is_active' in request.data:
            teacher.is_active = request.data.get('is_active')
        teacher.save()
        return Response({
...
def close_feedback_session(request, pk):
    """
    HOD/Admin: Close a feedback session
    """
    if request.user.role not in ['hod', 'admin']:
        return Response({'error': 'Only HOD/Admin can close sessions'}, status=403)
    try:
        session = FeedbackSession.objects.get(pk=pk)
        session.close_session()
        return Response({'message': f'Session {session.name} closed successfully'})
    except FeedbackSession.DoesNotExist:
        return Response({'error': 'Session not found'}, status=404)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def department_analytics(request):
    return Response({"message": "Department analytics not implemented yet"})

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def branch_comparison_analytics(request):
    return Response({"message": "Branch comparison analytics not implemented yet"})
